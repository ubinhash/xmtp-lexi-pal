...
export const VocabProgress: React.FC = () => {
    ...
    const fetchVocab = async () => {
      ...
      try {
        const response = await fetch('https://api.studio.thegraph.com/query/111655/base-xmtp-lexipal/version/latest', {
          ...
          body: JSON.stringify({
            query: `{
              vocabLearneds(where: { user: "${userAddress}" }, orderBy: blockTimestamp, orderDirection: desc, first: 1000) {
                word
                progress
                goalId
                blockTimestamp
        ...
        const toAdd = ALL_VOCAB.filter(word => !learnedWords.has(word)).slice(0, 15).map(word => ({ word, progress: 0 }));
  ...
  return (
    <div className={styles.card}>
      <div className={styles.header}>Vocab Progress</div>
